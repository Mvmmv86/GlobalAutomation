...
class _uq_constraint_sig(_constraint_sig):
    ...
    def __init__(self, const: UniqueConstraint, impl: DefaultImpl) -> None:
        ...
        self.sig = ("UNIQUE_CONSTRAINT",) + impl.create_unique_constraint_sig(const)
...
def _compare_indexes_and_uniques(
    ...
    metadata_indexes_sig = {_ix_constraint_sig(ix, impl) for ix in metadata_indexes}

    conn_unique_constraints = {_uq_constraint_sig(uq, impl) for uq in conn_uniques}
    ...
    metadata_uniques_by_sig = {uq.sig: uq for uq in metadata_unique_constraints_sig}
    ...
    for existing_name in sorted(set(metadata_names).intersection(conn_names)):
        ...
        if conn_obj.is_index != metadata_obj.is_index:
            ...
            if conn_obj.sig != metadata_obj.sig:
                msg.append(" expression %r to %r" % (conn_obj.sig, metadata_obj.sig))
