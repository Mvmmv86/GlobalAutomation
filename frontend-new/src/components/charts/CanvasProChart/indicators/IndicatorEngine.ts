/**
 * IndicatorEngine - Calcula TODOS os 30+ indicadores técnicos
 * Sistema PROFISSIONAL usando technicalindicators library
 */
...
// Import opcional do Worker Manager
import { IndicatorWorkerManager } from '../workers/IndicatorWorkerManager'

export class IndicatorEngine {
  private workerManager?: IndicatorWorkerManager
  ...
  constructor(options?: { useWorker?: boolean }) {
    ...
    if (this.useWorker) {
      try {
        this.workerManager = new IndicatorWorkerManager({
          onCalculateComplete: (result, time) => {
            console.log(`📊 Indicator ${result.type} calculated in ${time.toFixed(2)}ms (Worker)`)
          },
          onBatchComplete: (results, time) => {
            console.log(`📊 ${results.length} indicators calculated in ${time.toFixed(2)}ms (Worker)`)
          },
          onError: (error) => {
            console.error('❌ Indicator Worker error:', error)
          }
        })
        console.log('✅ IndicatorEngine using Worker for calculations')
      } catch (error) {
        console.warn('⚠️ Could not initialize Worker, falling back to main thread:', error)
   ...
   * Usa Worker se disponível para melhor performance
   */
  async calculate(config: AnyIndicatorConfig, candles: Candle[]): Promise<IndicatorResult | null> {
    ...
    // Se o Worker estiver disponível, usa ele
    if (this.useWorker && this.workerManager) {
      try {
        return await this.workerManager.calculate(config, candles)
      } catch (error) {
        console.warn('Worker calculation failed, falling back to main thread:', error)
        // Continua com o cálculo na main thread
      }
    }

    // Cálculo na main thread (fallback ou padrão)
  ...
  async calculateMultiple(configs: AnyIndicatorConfig[], candles: Candle[]): Promise<IndicatorResult[]> {
    // Se o Worker estiver disponível, usa ele para batch
    if (this.useWorker && this.workerManager) {
      try {
        return await this.workerManager.calculateBatch(configs, candles)
      } catch (error) {
        console.warn('Worker batch calculation failed, falling back to main thread:', error)
        // Continua com o cálculo na main thread
      }
    }
  ...
  /**
   * Calcula indicador de forma síncrona (main thread)
   */
  private calculateSync(config: AnyIndicatorConfig, candles: Candle[]): IndicatorResult | null {
    if (!config.enabled || candles.length === 0) {
      return null
    }
  ...
  private calculateVP(config: VPConfig, candles: Candle[]): IndicatorResult {
    ...
    // Volume Profile retorna estrutura diferente, simplificando para array
  ...
  isUsingWorker(): boolean {
    return this.useWorker && !!this.workerManager?.isUsingWorker()
  }

  /**
   * Destrói o Worker se estiver em uso
   */
  destroy(): void {
