#!/usr/bin/env python3
"""
Teste dos webhooks BTC e ETH COM assinatura HMAC correta
"""
...
def generate_hmac_signature(payload_dict, secret):
    """Generate HMAC signature for payload"""
    # Normalize payload (same as service does)
    normalized_payload = json.dumps(payload_dict, separators=(",", ":"), sort_keys=True)
...
async def test_webhook(webhook_name, payload):
    """Test a specific webhook with proper HMAC signature"""
    ...
    try:
        ...
        print(f"   Secret: {'***' + webhook.secret[-4:] if webhook.secret else 'NENHUM'}")
        ...
        if webhook.secret:
            ...
            print(f"\n🔐 HMAC Signature gerada: {signature[:20]}...")
        else:
            print(f"\n⚠️  Webhook SEM secret - sem HMAC")
...
async def main():
    print("=" * 80)
    print("TESTE DOS WEBHOOKS BTC E ETH (COM HMAC)")
    print("=" * 80)
...
if __name__ == "__main__":
    print("\n🧪 Iniciando testes dos webhooks BTC e ETH (com HMAC correto)\n")
